...
class Magic extends Hero {
  ...
  void applySuperPower(Boss boss, List<Hero> heroes) {
    ...
    if (RpgGame.roundNumber <= 4) {
      ...
      if (hero.isAlive()) {
        hero.damage += boosting;
      }
    }
    }
  }
  }
  //Не поняла нужен ли откат для баффа, поэтоиу сделала без отката
...
class Golem extends Hero {
  Golem(int health, int damage, String name)
    : super(health, damage, name, SuperAbility.blockDamage);

  @override
  void applySuperPower(Boss boss, List<Hero> heroes) {
    int redirectedDamage = boss.damage ~/ 5;
    print('$name intercepted $redirectedDamage damage from the boss.');
  }
}
...
class Thor extends Hero {
  ...
  @override
  void applySuperPower(Boss boss, List<Hero> heroes) {
    if (!boss.isStunned && RpgGame.random.nextBool()) {
  boss.isStunned = true;
